...
def serialize_post(post):
    ...
    return {
        ...
        'slug': post.slug,
        'tags': [serialize_tag(tag) for tag in post.tags.prefetch_related(Prefetch('posts', queryset=queryset))],
...
def index(request):
    ...
    fresh_posts = Post.objects.order_by('published_at').prefetch_related('author', 'tags').annotate(
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all()[:20].prefetch_related('author', 'tags').annotate(num_comments=Count('comments'))
